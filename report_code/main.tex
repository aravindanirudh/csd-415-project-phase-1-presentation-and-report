...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{College of Engineering, Cherthala}}
\fancyfoot[L]{\small Department of Computer Science and Engineering}
\fancyfoot[R]{\small\thepage}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\footrulewidth}{0pt}

% Plain style used by chapter-opening pages
...
% Front-matter style (roman page numbers, centred)
...
{\normalsize\bfseries ANANDHAKRISHNAN S \quad (CEC23CS033)}\\[0.1cm]
{\normalsize\bfseries ARAVIND A KAMATH \quad\; (CEC23CS041)}\\[0.1cm]
{\normalsize\bfseries ARJUN MANOJ \qquad\qquad (CEC23CS043)}\\[0.1cm]
...
\vspace*{\fill}
\begin{flushright}\small i\end{flushright}
...
The rapid proliferation of fake news and misinformation across social media and digital
platforms has emerged as a critical societal challenge. Fabricated news heavily influences
democratic elections, promotes harmful medical myths and incites communal unrest.
Existing automated detection systems achieve high statistical accuracy but function as
opaque black boxes, producing verdicts without any explanation of the reasoning behind
them. This absence of transparency limits institutional credibility and prevents everyday
users and journalists from trusting automated classifications.

This project proposes \textbf{Ledger}, an AI-powered web application for real-time fake
news detection accompanied by transparent, word-level reasoning. The core classification
engine uses \textbf{FastText} subword embeddings to capture semantic meaning and
handle out-of-vocabulary terms, typos and internet slang. A \textbf{1D Convolutional
Neural Network (CNN)} extracts localized n-gram features, which are passed into a
\textbf{Long Short-Term Memory (LSTM)} network that models sequential contextual
dependencies across the article. To address the black-box limitation, the system
integrates Explainable AI using \textbf{LIME} (Local Interpretable Model-agnostic
Explanations), visually highlighting the specific words that drove the Real or Fake
verdict together with calibrated confidence scores.

The selected base paper, \textit{Advancing Fake News Detection: Hybrid Deep Learning With
FastText and Explainable AI} (Hashmi et al., IEEE Access, 2024), establishes the
high performance of FastText-CNN-LSTM architectures, achieving up to 99\,\% accuracy
on the WELFake benchmark. The proposed implementation connects a Python
\textbf{FastAPI} backend for low-latency model serving to a responsive
\textbf{React.js} frontend dashboard. This report presents the proposed system design
and a comprehensive literature survey; it does not claim completed deployment or
